...
class Shrink:
    ...
    def getResiduals(self):
        ...
        csv_file_path = "../Data/Compressed/residuals/residuals.csv"
    ...
    def residualEncode(self, residuals, epsilon):
        ...
        outputPath = '../Data/Compressed/residuals'
        InFilePath = "../Data/Compressed/residuals/QuantiresdiaulsVal.csv"
        ...
        residualSize = os.path.getsize('../Data/Compressed/residuals/codes.rc')
    ...
    def residualDecode(self, outputPath, epsilon):
        ...
        csv_file_path = "../Data/Compressed/residuals/DeQuanresdiauls.csv"
        with open(csv_file_path, mode='w', newline='') as csv_file:
            csv_writer = csv.writer(csv_file)
    ...
    def dynamicEpsilon(self, startIdx, points):
        ...
        C = round(math.exp((2/3 - (local_max - local_min)/ (self.max-self.min))), 3)
    ...
    def quantization(self, value, localEpsilon):
        #precision = len(str(localEpsilon).split('.')[1])
        res = round(value / localEpsilon ) * localEpsilon
        #res = float(format(res, f'.{precision}f'))
        #print(f"value = {value}, localEpsilon = {localEpsilon}， newvalue = {res}")
        return res
    ...
    def createSegment(self, startIdx, points, segments, localEpsilon):
        ...
        # LOFcontainer = []
        # LOFcontainer.append([startIdx, points[startIdx].value])
        ...
        if startIdx + 1 == len(points): # Case1: only 1 ponit
            ...
            # return startIdx + 1, segment, LOFcontainer
            return startIdx + 1, segment
        
        # LOFcontainer.append([startIdx+1, points[startIdx+1].value])
        ...
        if startIdx + 2 == len(points): # Case2: only 2 ponits
            ...
            # return startIdx + 2, segment, LOFcontainer
        ...
        # LOFcontainer.append([idx, points[idx].value])
        segment = SimPieceSegment(initTimestamp, aMin, aMax, b)
        segments.append(segment)

        # return len(points), segment,LOFcontainer
    ...
    def toByteArrayPerBSegments(self, segments: List[SimPieceSegment], variableByte: bool, outStream: io.BytesIO) -> None:
        ...
        filename = 'Base.csv'
        with open('../Data/Compressed/Base/'+filename, 'w', newline='') as csvfile:
    ...
    def saveByte(self, byts, filename):
        ...
        path = '../Data/Compressed/Base/'+filename[:-7]+"_Base.bin"
